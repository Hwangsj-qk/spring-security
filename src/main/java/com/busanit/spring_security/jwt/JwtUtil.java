...
public class JwtUtil {
    ...
    private final long expiration = 86400000;        // 토큰 만료기간(24시간)
    ...
    public <T> T extractClaim (String token, Function<Claims, T> claimResolver) {
        Claims claims = Jwts.parserBuilder().setSigningKey(getStringKey())
                .build()
    ...
    public String extractUsername(String token) {
        return extractClaim(token, Claims::getSubject);      // Subject : 사용자 이름
    }
    ...
    private Boolean isTokenExpired(String token) {
        // 현재날짜와 비교하여 토큰의 만료날짜가 이전인지에 따라 true / false 반환
    ...
    private String createToken(Map<String, Object> claims, String subject) {
        return Jwts.builder()
                ...
                .setIssuedAt(new Date(currentTimeMillis()))      // 토큰 발급시간 = 현재시간
                .setExpiration(new Date(currentTimeMillis() + expiration))   // 토큰 만료 시간
                .signWith(getStringKey(), SignatureAlgorithm.HS256)         // 비밀키, 알고리즘
                .compact();     // 생성된 토큰을 문자열로 컴팩트
    }
